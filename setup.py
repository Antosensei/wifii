...
# code for checking if openssl library exists
OPENSSL_CODE = dedent("""
...
def check_required_library(libname, libraries=None, include_dir=None):
    ...
    err_msg = (
        ...
        + libname
        + " is required for the compilation of roguehostapd. "
        + "Please install it and rerun the script (e.g. on Debian-based systems "
        + "run: apt-get install "
    )
    if libname == "openssl":
        err_msg += "libssl-dev"
...
def check_dnsmasq():
    ...
    if not os.path.isfile("/usr/sbin/dnsmasq"):
        sys.exit(
            ...
            + "(e.g. on Debian-based systems: apt-get install dnsmasq)"
        )
